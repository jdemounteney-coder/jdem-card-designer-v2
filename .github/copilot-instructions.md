...
- **Project type:** Create React App single-page app using React 19 and `react-scripts` (see [package.json](../package.json)).
- **Entry point:** [src/index.js](../src/index.js) — renders the main app component.
- **Primary UI & logic:** [src/App.js](../src/App.js) — single large component `PlayingCardDesigner` that holds most state and behavior.

Big picture (quick):
- Users upload images (suit symbols, number glyphs, face art, and a card back). These are stored as data-URIs in component state (`suits`, `numbers`, `faceCards`, `cardBack`). See the state declarations and upload handlers in [src/App.js](../src/App.js).
...
- Styling uses Tailwind: see `src/index.css` (contains `@tailwind` directives) and `devDependencies` in [package.json](../package.json).
...
- File input handling: The app supports click, drag, and paste. Look for `handlePaste`, `handleDrop`, and `FileReader.readAsDataURL` usages in [src/App.js](../src/App.js).
